import torch
from custom_vision_transformer import vit_custom, vit_b_16, ViT_B_16_Weights
...
model = vit_custom(num_classes=params.num_classes, image_size=params.img_size, attention_bias=attention_bias).to(device)
...
for epoch in tqdm(range(1, params.num_epochs + 1), desc="Epochs"):
    ...
    if epoch % 10 == 0 and save_checkpoints: # Save the model checkpoint every 10 epochs
        ...
        torch.save(checkpoint, f"./training_checkpoints/vit_custom_epoch_{epoch}.pth")
